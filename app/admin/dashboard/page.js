...
export default async function DashboardPage() {
  ...
  const [postCount, videoCount, subscriberCount] = await Promise.all([
  prisma.post.count(),
  prisma.video.count(),
  ...
  className="bg-white border border-brand-100 rounded-2xl p-6 hover:shadow-md transition"
>
  <p className="text-sm text-brand-900/60">মোট Subscriber</p>

  <p className="text-4xl font-bold text-brand-700 mt-2">
    {subscriberCount}
  </p>
